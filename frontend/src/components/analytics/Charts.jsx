...
/**
 * 1. TrendAreaChart
 * A responsive area-trend chart with gridlines and interactive tooltip.
 * Data format: [{ label: string, value: number, details?: { [key: string]: number } }]
 */
export function TrendAreaChart({ data = [], height = 240, lineColor = "#10b981", areaColor = "rgba(16, 185, 129, 0.15)" }) {
  ...
  const maxVal = Math.max(...values, 5); // Fallback to 5 if empty
  ...
  const points = data.map((item, idx) => {
    const x = padding.left + (idx * chartWidth) / Math.max(1, data.length - 1);
  ...
  const handleMouseMove = (e) => {
    ...
    // Convert mouseX to SVG coordinate scale
    ...
    // Compute tooltip position relative to client page bounding rect
    const tooltipX = points[closestIdx].x;
    const tooltipY = points[closestIdx].y - 10;
  ...
  return (
    ...
    >
      <svg className="chart-svg" viewBox={`0 0 ${svgWidth} ${svgHeight}`} width="100%">
        ...
        {yTicks.map((tickVal) => {
          ...
          return (
            <g key={tickVal}>
              ...
              <text className="chart-axis-text" x={padding.left - 10} y={y + 4} textAnchor="end">
                {tickVal}
              </text>
            </g>
          );
        ...
        {/* X axis labels */}
        {data.map((item, idx) => {
          if (data.length > 12 && idx % Math.ceil(data.length / 6) !== 0) {
            return null; // Skip some labels if too many data points
          }
          const x = padding.left + (idx * chartWidth) / Math.max(1, data.length - 1);
          return (
            <text
        ...
        {/* Shaded Area */}
        {areaStr && <path className="chart-area-fill" d={areaStr} fill="url(#trendAreaGradient)" />}
        ...
        {points.map((p, idx) => (
          <circle
            ...
            fill={hoveredIndex === idx ? lineColor : "#ffffff"}
            stroke={lineColor}
      ...
      {hoveredIndex !== null && (
        ...
        >
          <div className="chart-tooltip-title">{data[hoveredIndex].label}</div>
          <div className="chart-tooltip-row">
            <span className="chart-tooltip-label">Sales:</span>
            <span className="chart-tooltip-val">{data[hoveredIndex].value} units</span>
          </div>
          {data[hoveredIndex].details && Object.keys(data[hoveredIndex].details).length > 0 && (
            <div style={{ marginTop: "6px", paddingTop: "4px", borderTop: "1px dashed rgba(255,255,255,0.15)" }}>
              {Object.entries(data[hoveredIndex].details).map(([prod, qty]) => (
                <div key={prod} className="chart-tooltip-row" style={{ fontSize: "0.75rem" }}>
                  <span className="chart-tooltip-label">{prod}:</span>
                  <span className="chart-tooltip-val" style={{ color: "#38bdf8" }}>{qty}</span>
                </div>
              ))}
            </div>
          )}
